...
def calculate_metrics():
    ...
    idx = 0
    print(sentence_metrics(lines[idx], struct_drs[idx], debug=True))
...
def sentence_metrics(sent, struct, debug=False):

    if debug:
        print("Sent: ", sent.strip())
        print("Struct: ", struct )
        print(f"Sent. length: ", len(sent))
        print(f"Struct. length", len(struct))
